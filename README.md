...
1. 絶対にMarkdown記法を使用しない
   - **（太字）、##（見出し）、-（箇条書き）などの記号は使わない
   - 箇条書きは「・」または番号（1. 2. 3.）を使用する

2. 人間が書いたような自然な文章にする
   - 「分析によると」「データに基づき」などAI的な表現を避ける
   - ビジネスパーソンが書いたような具体的で断定的な文体にする
...
**編集例:**
- Markdown記号の禁止ルールを追加/変更
- AI的な表現の禁止リストを追加
- 出力フォーマットの指定を変更

### 個別プロンプトの編集

特定のテンプレートのみに適用される指示を変更したい場合：

1. 対象のテンプレートシート（①〜⑤など）を開く
2. A3セルの内容を編集
3. 保存（自動保存）
